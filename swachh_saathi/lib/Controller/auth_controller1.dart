...
class AuthController1 extends GetxController
  ...
  var statusMessageColor = Colors.white.obs;
  ...
  checkLoginStatus(User? user) async {
    ...
    if (firstTimeLogin) {
      ...
      Get.to(() =>  LoginScreen()); // Navigate to intro screen on first time login
    } else {
      if (user == null) {
        Get.to(() => LoginScreen());
      } else {
  ...
  verifyOTP() async {
    try {
      ...
      // collection users doc(userCredential.user!.uid) set(phone, userId) setMergeOptions true
